...
public class ExportExcelUtil<T> {
    ...
    // 大于11位时，会产生科学计数法
    private static final Long SCIENTIFIC_NOTATION_CRITICAL_VALUE = 99999999999L;
    ...
    public void excelData() {
        List<Object> objectDatas = Lists.newArrayList();
        BigDecimal one = new BigDecimal(1.23);
        objectDatas.add(one);
        String two = "test";
        objectDatas.add(two);
        Integer three = 2;
        objectDatas.add(three);
        for (Object obj : objectDatas) {
            if (obj instanceof BigDecimal) {
                System.out.println("bigdecimal=" +  NumberUtils.toBigDecimal(obj, 2));
    ...
    public HSSFCell cteateCell2(HSSFWorkbook workBook, HSSFRow row, int col, String val, HSSFCellStyle cellStyle) {
        HSSFCell cell = row.createCell(col);
        if (StringUtils.isNotEmpty(val) && com.jef.util.StringUtils.isBigDecimal(val) && NumberUtils.isLessThan(val, SCIENTIFIC_NOTATION_CRITICAL_VALUE)) {
            // 数字
            cell.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
            if (StringUtils.isNumeric(val)) {
                cell.setCellValue(Integer.valueOf(val));
                // 整数
                cell.setCellValue(NumberUtils.getParaDouble(val, 0));
                // 这三行可以忽略
                /*if (LogicUtils.isNull(cellStyle)) {
                    cellStyle = workBook.createCellStyle();
                }
                cellStyle.setDataFormat(HSSFDataFormat.getBuiltinFormat("0"));*/
            } else {
                // 浮点数
                cell.setCellValue(NumberUtils.getParaDouble(val, com.jef.util.StringUtils.getDigit(val)));
            }
        } else {
            // 字符串
        ...
        if (LogicUtils.isNotNull(cellStyle)) {
