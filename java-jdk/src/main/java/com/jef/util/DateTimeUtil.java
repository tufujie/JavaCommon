...
public class DateTimeUtil extends DateUtils {
     ...
     * @return
     */
    public static Date convertAngelesDateToChina(String timeStr) {
        return convertDateToChina(timeStr,"America/Los_Angeles");
    }
    /**
     ...
     * @return
     */
    public static Date convertDateToChina(String timeStr, String timezone) {
        Date cnDate = null;
        if(StringUtils.hasText(timeStr)){
            timeStr = timeStr.trim().replaceAll(" ","T");
            DateTime usDate = new DateTime(timeStr, DateTimeZone.forID(timezone));
            String cnDateS = usDate.withZone(DateTimeZone.forID("Asia/Shanghai")).toString(DEFAULT_DATE_FORMAT);
            cnDate = DateTime.parse(cnDateS, DateTimeFormat.forPattern(DEFAULT_DATE_FORMAT)).toDate();
        }
        return cnDate;
    }
