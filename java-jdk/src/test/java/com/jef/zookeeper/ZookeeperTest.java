...
public class ZookeeperTest {
    ...
    public void testCreatePersistentNode() throws Exception {
        ...
        if (stat == null) {
            zookeeperObj.create(nodePath, "123".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        }
        /**
         * 同步创建一个持久节点，ACL为 world:anyone:cdrwa 所有人只拥有创建的权限,等同于如下命令：
         * create /node1 123 world:anyone:c
         */
        String nodePath2 = "/node1";
        Stat stat2 = zookeeperObj.exists(nodePath2, true);
        if (stat2 == null) {
            zookeeperObj.create(nodePath2, "123".getBytes(), Collections.singletonList(new ACL(ZooDefs.Perms.CREATE, ZooDefs.Ids.ANYONE_ID_UNSAFE)), CreateMode.PERSISTENT);
        }
    }
     ...
     */
    @Test
    public void testCreateEphemeraltNode() throws Exception {
        ZooKeeper zookeeperObj = ZookeeperUtil.getZookeeper();
    ...
    public static void main(String[] args) throws Exception {
         ...
         */
        zookeeperObj.addAuthInfo("digest", "wangsaichao:123456".getBytes());
         ...
         *
         */
        byte[] data = zookeeperObj.getData("/node3", new ZookeeperWatcher(), new Stat());
         ...
         * ctx 回调上下文
         */
        zookeeperObj.getData("/node3", false, new AsyncCallback.DataCallback() {
            @Override
         ...
         * 同步设置数据 -1匹配任何版本
         */
        Stat stat = zookeeperObj.setData("/node3", "嗨喽".getBytes(), -1);
        System.out.println(stat);

        /**
         * 异步设置数据
         */
        zookeeperObj.setData("/node3", "helloword".getBytes(), -1, new AsyncCallback.StatCallback() {
            @Override
         ...
         */
        zookeeperObj.delete("/node1", -1);

        /**
         * 异步删除节点
         */
        zookeeperObj.delete("/node2", -1, new AsyncCallback.VoidCallback() {
            @Override
         ...
         * 同步检查节点是否存在,并留下监听
         */
        Stat exists = zookeeperObj.exists("/node2", new Watcher() {
            @Override
         ...
         * 异步检查节点是否存在,并留下监听
         */
        zookeeperObj.exists("/node2", new Watcher() {
            @Override
            public void process(WatchedEvent event) {
                System.out.println("留下监视");
            }
        ...
        zookeeperObj.addAuthInfo("digest", "helloworld:123456".getBytes());
        //因为是 /node3 节点 所以还需要 先添加 /node3的授权
        zookeeperObj.addAuthInfo("digest", "wangsaichao:123456".getBytes());
        Stat auth = zookeeperObj.setACL("/node3", Collections.singletonList(new ACL(ZooDefs.Perms.ALL, new Id("auth", "helloworld:123456"))), -1);
         ...
         * 获取ACL
         */
        List<ACL> acl = zookeeperObj.getACL("/node3", new Stat());
